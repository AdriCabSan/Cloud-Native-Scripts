...
aws cloudfront list-distributions --output json --query 'DistributionList.Items[*].[{Id:Id,ARN:ARN,Origins.Items[*].DomainName:DomainName} | [0],DefaultCacheBehavior.LambdaFunctionAssociations.Quantity>`0` && DefaultCacheBehavior.LambdaFunctionAssociations.Items[*].{LambdaFunctionArn:LambdaFunctionARN} | [0],ViewerCertificate.ACMCertificateArn,ViewerCertificate.{Certificate:Certificate}] | []' | tee aws_cloudfront_distributions.txt

echo "###list all subnets####" #for getting an eye on general architecture in a region/account.
aws ec2 describe-subnets  --query 'Subnets[*].[VpcId,SubnetId,AvailabilityZone] | []' | grep '[^\s]' | tee aws_subnets.txt

echo "###List all security groups####"
aws ec2 describe-security-groups --query 'SecurityGroups[*].[GroupName,IpPermissions[*] | [].IpRanges[*].CidrIp | [],GroupId,VpcId]' |  grep '[^\s]' | tee aws_security_groups.txt

echo "#####List all Hosted Zones######"
aws route53 list-hosted-zones-by-name --query "HostedZones[*].[Id] | []" | grep hosted | tee aws_hostedZones.txt
...
echo "######list all roles############"
aws iam list-roles --query 'Roles[*].Arn' |  grep '[^\s]' | tee aws_roles.txt
